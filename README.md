...
现在有三种设备，分别是
1.奥比中光gemini215相机模组，用来分时采集color、左ir、右ir，最多有三个模组，分别在相机1、2、3显示彩色图；
2.迈德威视双目相机模组，用来采集左右灰度图，具体调用逻辑参照我上传的python程序，只需要触发保存左右各1帧即可；
3.海康威视相机，用来采集彩色图，在相机4显示彩色图；
软件启动，检测可连接的设备列表并更新。当点击触发按钮时，对于设备列表中的设备进行软件触发：
其中，奥比中光gemini215相机先触发保存最近的彩色帧数据，然后停流，将config设置为只开启ir流且channel设置为0获取左ir数据，然后停流切换channel为1再开启流获取右ir数据，等获取完毕再停流设置config只开启彩色流并恢复预览模式；海康相机和迈德威视相机直接下发触发命令保存触发帧。
要注意，要保证设备列表中的所有设备内部时钟基准一致，采用不断获取设备时间戳同步host时间的方法，以此来判断所有设备相机曝光时的时间戳差距。